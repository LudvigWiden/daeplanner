...
void AEPlanner::execute(const aeplanner::aeplannerGoalConstPtr& goal)
{
  ...
  if (root->gain_ > 0.25 or !root->children_.size() or root->score(params_.lambda) < params_.zero_gain)
    expandRRT();
  else
  ...
  ROS_DEBUG_STREAM("extractPose");
  result.pose.pose = vecToPose(best_branch_root_->children_[0]->state_);
  ROS_DEBUG_STREAM("Best node score: " << best_node_->score(params_.lambda));
...
RRTNode* AEPlanner::initialize()
{
  ...
  {
    ROS_DEBUG_STREAM("best_branch_root_");
  ...
  {
    ROS_DEBUG_STREAM("Initialize without any previous branch");
...
std::pair<double, double> AEPlanner::getGain(RRTNode* node)
{
  ...
  */
  ...
  ROS_DEBUG_STREAM("gain expl: " << ret.first);
  return ret;
}
...
void AEPlanner::agentPoseCallback(const geometry_msgs::PoseStamped& msg)
{
  ...
  //ROS_INFO_STREAM("x: " << current_state_[0] << " y: "<< current_state_[1] << " z: "<< current_state_[2] << " yaw: "<< current_state_[3]);
