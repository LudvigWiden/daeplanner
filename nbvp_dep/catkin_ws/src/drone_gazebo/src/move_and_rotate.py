...
from interface_nbvp_rotors.msg import Goal
import math
import numpy as np
...
SAFE_DISTANCE = 1.2 # How close we look for danger (dynamic obstacles)
AVOIDANCE_DISTANCE = 0.1 # The length that we move away from danger
...
def callback(all_states, goal):
	...
	# We have reached our goal position, 
	# Check if we have started rotation else stop the linear velocity		
	elif (not rotate):
		# Rotation is not started, stop the linear movement
		# Twist message
